...
| C2 ✅접근확인 | **취약성 곡선 미보정** — 불확실성의 **83%** 발생원(Sobol 실측). 기본 곡선은 글로벌/HAZUS(미국) | 국내 손실자료를 넣지 않음 | 행정안전부 **재해연보**(지역·재해별 피해액, 공개) + EM-DAT 한국 + 보험 손해통계 → `calibration` 실행(현재 TC `v_half` 대상)으로 피해곡선 재적합. 데이터 희소 시 베이지안 사전분포 접근 | 보정 러너 존재(EM-DAT 경로: `CLIMATERISK_EMDAT_PATH`). 2026-09-07부터 결과가 `data/calibrations/`에 출처·기간·목적함수·방법·경계·시각과 함께 저장되고, `tc_impf_default="calibrated"` 옵션으로 이후 실행에 적용됨(`fit_status="fitted"`, 검증 아님). **재해연보 로더는 미작성**(러너는 EM-DAT CSV만 읽음). 그 전까지 기본값은 Eberenz WP4 지역 프리셋(190.5) |
