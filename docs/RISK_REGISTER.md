...
| B2 | **폭염 사망의 검증 불가(합성 데이터)** | E-OBS 관측 연결로 2022년 최극단 재현 검증 (`test_observed_grid_reproduces_the_real_ranking…`) | 극단 연도 ~2.3배 과소예측 (격자 평활화·UHI·귀속창 차이) | MoMo 5~9월 창 통일 + β 연도별 시계열 재적합 + UHI 보정. **한국 적용 대상 아님**(검증 인프라 부재) |
