...
|---|---|---|---|---|
| C1 🟡데이터일부 | **해저드 국내 해상도 부재** — 전지구 이벤트셋의 격자·통계가 한국 지형(산악·도시)을 평활화 | 2026-09-11 **실제 기상청 남한상세 1 km 파일로 실행 완료**. 남은 것은 관측 손실 검증(C5)과 2050 이후 창 | ① 기후변화 상황지도에서 남한상세 격자 평균기온(TA) 일자료 → ② `kma_scenario.load_summer_tmax` → ③ `heat_mortality.grid_from_summer_tmax` → ④ `hazard_convert.convert_grid_to_catalog` → 카탈로그 등록 → 러너가 자동 우선 조회(없으면 `historical` 폴백, 폴백 사실을 `detail`에 명시). 실행 결과: MK-PRISM v3.1 2000–2019(nc) + SSP2-4.5·SSP5-8.5 2021–2030(asc), 0.05° **4,312셀**, 레이어 6개 등록. 관측 6.7 DD/셀·시즌 vs SSP2-4.5 24.4 / SSP5-8.5 25.9(2021–2030). **검증: 관측 구간에서 2018년이 2위의 3.7배로 1위**(홍천 41.0 °C 역대최고·서울 폭염 48일), 최하위 2003년(최근 최냉 여름) — 모델에 어떤 연도도 맞춰 넣지 않았으므로 독립 검증. `test_real_kma_files_reproduce_the_ranking_of_korean_summers` | 구현 시 실제 파일이 사양과 3곳 달랐고 모두 코드로 흡수: ⓐ AR6 남한상세는 **nc 없이 asc만** 제공(헤더 없는 1줄=1일, 751×601 row-major, 윤년 366줄; 방향은 NetCDF 마스크와 98.2% 일치로 확정) — 시나리오당 13 GB라 tar를 스트리밍해 6–9월 122줄만 파싱하고 디스크에 쓰지 않음. ⓑ 파일명에 `skorea` 토큰이 없는 산출물(MK-PRISM)과 **연도 1개씩인 멤버**. ⓒ 관측은 **0.005°(1201×1501)**, 시나리오는 0.01° — 짝수 노드가 일치하므로 서브샘플링으로 정합(블록평균은 중심이 0.0025° 어긋나 기준밴드 상속이 거부). 또한 두 산출물의 육지 마스크가 달라(4,691 vs 4,326) 모든 창을 `common_footprint` 교집합에 고정 — 그러지 않으면 시나리오 차이에 '어느 셀을 셌는지'가 섞임 |
...
2. ~~**C1 해저드 주입**~~ — **2026-09-11 완료.** 남은 것은 2031–2060 asc 창 추가 다운로드로 2050 앵커를 채우는 것뿐이며(코드 변경 없음), 목록은 `KMA_DOWNLOAD_LIST.md`.
...
| C1 | **기상청 SSP 남한상세 1km** (기후정보포털 → 2025년부터 **기후변화 상황지도**로 이관) | ✅ 무료(확인) | 이용목적·분야·소속 입력 + 동의 후 '데이터 다운로드 신청'. **기후정보포털 자체 다운로드는 2024-07-01 종료**(활용매뉴얼 v5.1 명시), 상황지도에서 제공 | NetCDF·ASCII(tar.gz). **격자 확정: 751(경도)×601(위도), 0.01°(≈1 km), 시작 (33.0N, 124.5E), 위경도 직각좌표 WGS84, 결측 -9990.** 변수 TA/TAMAX/TAMIN/RN/RHM/WS/SI, 일/월/연. 과거 MK-PRISM **v3.1** **2000–2019**(nc), 미래 **2021–2100**(남한상세 시나리오는 **asc만** 제공). SSP126/245/370/585 × 5개 RCM(HadGEM3-RA·WRF·CCLM·GRIMs·RegCM4) + 앙상블(5ENSMN) | ✅ 정규 위경도 격자라 `hazard_convert`에 재투영 없이 직결. **TA(일평균) daily** → 폭염 degC-days(인용 가능한 임계값이 모두 일평균 기준), RN daily → 극한강수 | 공공기관 정책 수립 시 **의무 사용** 표준 시나리오. 파일명 예 `AR6_SSP585_5ENSMN_skorea_TA_gridraw_daily_2021_2030_asc.tar.gz` / `MKPRISM_MKPRISMv31_TA_gridraw_daily_2000_2019_nc.tar.gz` (MK-PRISM은 `skorea` 토큰이 없고, 아카이브 멤버는 연도 1개씩) |
