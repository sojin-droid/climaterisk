...
| C5 🟡관측확보·검증전 | **국내 검증 기준 부재** — 모델값을 대조할 실측 손실 | **관측 계열 확보됨(2026-09-11)**: 재해연보 전국 태풍 피해 2016–2023(8년, 비영 7년, 백만원, `national:KOR`, nominal)을 F1 로더(`observed_kr.py`)가 API에서 적재한다. **집계 vs 바람 전용 불일치 확인됨**: 관측은 `wind+surge+rain`, 러너는 `wind`만 — `comparability_report`가 단위·커버리지·범위 3건으로 차단한다. **연도별 진단 완료**: 관측 트랙(`event_type=observed`, 1980–2020, 전부 `orig=True`)으로 교집합 2016–2020(n=5) 비교를 `scripts/tc_capture_kr.py`가 재현 가능하게 산출한다. **보정은 계속 차단**이며 이 진단으로 해제하지 않는다. **절대 포착률은 산출하지 않음** — 인용 가능한 한국 자산스톡 수치가 없어 노출을 1.0으로 정규화했고, 모델 손실은 통화가 아니라 정규화 노출의 분율이다 | 남은 것: ① 인용 가능한 한국 자산스톡(한국은행 국민대차대조표 순고정자산) 확보 후에야 절대 포착률을 말할 수 있다 ② 관측 트랙이 2020에서 끝나 2021–2023은 연도별 비교에서 제외됨 ③ 커버리지 격차는 데이터가 아니라 모델 범위 문제라 해일·호우 성분 없이는 좁힐 수 없다 | 비교 프레임워크 존재(`validation.py`), **관측 계열 존재**, 진단 산출물은 `outputs/`에 쓰고 커밋하지 않는다(관측값 포함). 상세: `OBSERVED_LOSSES_KR_SPEC.md`, `CLIMADA_METHODS.md` §8.4 |
