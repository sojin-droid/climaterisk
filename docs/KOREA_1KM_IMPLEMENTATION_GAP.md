...
램프에 연결하며 `"daily Tmax"`로 라벨링했다. 램프는 UTCI/열지수 스케일이고, 레이어는 일평균 백분위였다. 세 안을 같은 8항목으로 정의했고, **2026-09-23에 Option C로
결정했다**(아래 "결정" 절). 세 안의 표는 결정의 근거 기록으로 남긴다.
...
| 필요 KMA 변수 | `TAMAX` — **2026-09-23 보유·배선** (MK-PRISM 2000–2019, SSP245/585 2021–2060, `~/climada/data/kma/`) |
| 시간 집계 | 일 → 시즌 초과일수/강도 |
| 공간 해상도 | 현 경로 그대로 |
...
| 현재 구현 | **구현(2026-09-23)** — `kma_scenario.HEATWAVE_VARIABLE = "TAMAX"`; `heat_korea.register_window`가 TAMAX를 별도로 읽어 `heatwave` 레이어만 만들고, TAMAX가 없으면 레이어를 만들지 않는다(TA로 근사 금지). `heat_mortality`는 `TA` 유지 |
| 다음 결정 | 램프 재정의는 **하지 않았다** — 곡선은 지시적 그대로이고 신규 엔진(`physical_risk`)에서 폭염은 `HAZARD_ONLY`(금액 없음). Tmax 기준 생산성 곡선의 근거가 생기면 그때 별도 단계 |

**공통 결론**: 세 안 모두 **새 해저드 정의 + 새 곡선 근거**를 요구한다. A만 현 램프의 스케일과 맞지만 램프의 값 자체는
어느 안에서도 근거가 없다.

### 결정 — Option C (2026-09-23)

| 항목 | 결정 내용 |
|---|---|
| 해저드 | `heatwave` = KMA `TAMAX`(일최고기온)의 **시즌(6–9월) 95백분위**, °C. `kma_scenario.HEATWAVE_VARIABLE = "TAMAX"` |
| 분리 | `heat_mortality`는 `TA`(일평균) 유지 — Kim 2020 계열 추정치가 일평균 기준. 두 레이어는 변수를 공유하지 않는다 |
| 근사 금지 | TAMAX 파일이 없는 창은 `heatwave` 레이어를 **만들지 않는다**(`heat_korea.register_window`). TA로 대체하지 않는다 |
| 정합성 | TA·TAMAX는 같은 footprint(교집합)·같은 시즌 집합에서만 짝을 이룬다(`_check_aligned`, `_intersect_footprints`). TAMAX는 **TA가 실제로 가진 시즌**(2030 창: 2021–2030)으로 고정해 읽는다 — TAMAX 2031–2060은 TA가 같은 구간을 갖기 전까지 쓰지 않는다 |
| 라벨 | 매니페스트 `source`의 "daily Tmax"가 이제 사실이다(`… KMA 남한상세 TAMAX …`). 현황 체크의 `label_variable_conflict`가 해소된다 |
| 금액 | **financial loss는 계속 N/A.** CLIMADA에 폭염 임팩트 함수가 없고, 이 결정으로 **임팩트 함수를 새로 만들지 않는다**. 신규 엔진에서 폭염은 `HAZARD_ONLY` |
| 램프 | 레거시 러너의 지시적 램프 `[0,30,35,40,45] °C`는 **재정의하지 않았다**. Tmax p95(관측 대략 30대 °C)는 이제 램프 구간에 닿지만, 램프의 값에 근거가 없다는 사실은 변하지 않는다 — 레거시 `heatwave` 결과는 여전히 indicative |
| 해상도 | 변경 없음 — 저장 0.05° (`5km-current`) |
| 선택하지 않은 안 | A(UTCI): RHM·WS·SI 미배선, 일평균 입력 UTCI의 근거 부재. B(TA 재정의): 새 곡선 필요는 동일하고 라벨 불일치를 고치지 못한다 |

이 결정으로 바뀐 것은 **해저드 변수 하나**다. 곡선·금액·해상도는 그대로다.
